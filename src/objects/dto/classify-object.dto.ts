...
import { Classification, ObjectDescription, PlotData } from '../classes';

export class ClassifyObjectDto {
  ...
  @ApiProperty({ required: false, description: 'Speed', example: 0 })
  speed?: number;
  ...
  @ApiProperty({ 
    description: 'Classification data with suggestion',
    example: {
  ...
  @ApiProperty({
    required: false,
    description: 'List of radars that detected this object (auto-calculated if not provided)',
  ...
  @ApiProperty({
    required: false,
    type: 'array',
    description: 'Follow-up steps with questions and answers to display sequentially',
    example: [
      {
        question: 'האם תרצה שאקריא בד"ח פעולה?',
        answers: ['תדריך פעולה מפורט...']
      },
      {
  ...
  @ApiProperty({
    required: false,
    description: 'Pre-formatted detailed message to send in chat (HTML allowed)',
    example: 'מטרה FDE145 התגלתה בלבנון לפני 12 שניות...'
  })
